...
class GamePickRepo(private val gamePickDao: GamePickDao) {
    ...
    suspend fun undoLastPick(
        ...
        return try {
            ...
            Log.d(
                "GAMEPICK_REPO",
                "🔄 Cofanie wyboru: ${lastPick.gameName} gracza ${lastPick.playerId}"
            )
            ...
            participants.forEach { participant ->
                Log.d(
                    "GAMEPICK_REPO",
                    "↻ Przywracanie gracza ${participant.playerId} na pozycję ${participant.queuePosition}"
                )

                // Przywróć pozycję z GameSessionParticipant do Player.queuePosition
                playerRepo.updatePlayerQueuePosition(
